        ...
        switch ($page) :
            ...
            default:
                ...
                if (!$permissions) {
                    ...
                    //Based on the session selected level, we decide which queries to run to grab the right groups for the table.
                    switch($level) {
                        case 'g':
                            $groups = $this->administration->getSelectedGroup($this->user['DropdownDefault']->SelectedGroup);
                        break;
                        case 'c':
                            $groups = $this->administration->getParentGroupOfClient($this->user['DropdownDefault']->SelectedClient);
                        break;
                        default:
            ...
            default:
                ...
                if (!$permissions) {
                    ...
                    foreach ($groups as $group) :
                        ...
                        //USE TO POST TO THE EDIT PAGE TO GRAB THE CORRECT AGENCY FROM THE DB.
                        $this->table->add_row(
                                $group->Name, $group->AgencyName, (($group->Status) ? 'Active' : 'Disabled'),
