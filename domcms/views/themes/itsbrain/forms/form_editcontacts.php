					...
					<?php
						...
						foreach ($contact->Email as $type => $email) {
							if ($email == $contact->PrimaryEmailType) { ?>
	                            <div class="formRight"><?php echo  form_input(array('class'=>'required validate[required,custom[email]]','name'=>'PersonalEmailAddress','id'=>'email','value'=>$email)); ?></div><div class="fix"></div>
								<?php break;
							}
						}
					?>
                	...
                	<?php
						...
						foreach ($contact->Email as $type => $email) {
							if ($email != $contact->PrimaryEmailType) { ?>
                    			...
                    			<div class="formRight"><?php echo  form_input(array('class'=>'validate[custom[email]]','name'=>'WorkEmailAddress','id'=>'email','value'=>$email)); ?></div><div class="fix"></div>
							<?php }
						}
					?>
                </div>
                        ...
                        <?php
                            ...
                            foreach ($contact->Phone as $type => $phone) {
                                if ($phone == $contact->PrimaryPhoneType) { ?>
                                    <div class="formRight"><?php echo  form_input(array('name'=>'DirectPhone','id'=>'phone','class'=>'maskPhone validate[required,custom[phone]]','value'=>$phone)); ?><span class="formNote">(999) 999-9999</span></div>
                                    <?php break;
                                }
                            }
                        ?>
                        ...
                        <?php
                            ...
                            foreach ($contact->Phone as $type => $phone) {
                                if ($phone != $contact->PrimaryPhoneType) { ?>
                                    ...
                                    <div class="formRight"><?php echo  form_input(array('name'=>$type.'Phone','id'=>'phone','class'=>'maskPhone validate[required,custom[phone]]','value'=>$phone)); ?><span class="formNote">(999) 999-9999</span></div>
                                <?php }
                            }
                        ?>
                    </div>
